...
- Functions with the =(limited)= suffix are implemented but do not support full configuration or match the complete native =libgit2= signature (e.g., defaulted options structs).
